...
// Enables or disables speculative decoding (MTP drafter).
//
// When enabled, a small drafter model predicts the next N tokens per decode
// step; the main model verifies them in a single forward pass. Accepted
// tokens are emitted together, yielding roughly 1.5-2x decode throughput on
// supported hardware. The drafter must be present in the .litertlm asset —
// Gemma 4 bundles one under the `tf_lite_mtp_drafter` section.
//
// Trade-off: the drafter materializes ~300-400 MB of extra weights into RAM
// while active. Disable on memory-constrained devices.
//
// @param settings The engine settings.
// @param enable True to enable speculative decoding; false to disable.
LITERT_LM_C_API_EXPORT
